...
for md in lectures/*.md; do
    if ! grep -q "draft:\s*true" $md; then
        ...
        # only process if the markdown is newer than the pdf
        if [ -e lectures/pdfs/$doc.pdf ]; then
            md_date=$(git log -1 --format=%ct $md)
            pdf_date=$(git log -1 --format=%ct lectures/pdfs/$doc.pdf)
            
            if [ $md_date -lt $pdf_date ]; then
                continue;
            fi
        fi
